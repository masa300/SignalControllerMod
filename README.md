...
- ## SignalControllerBlock
  ...
  - #### above
    制御する信号機が真上(32ブロック以内)にある場合はチェック[x]を付けるだけでリンクさせることができます。<br>
    ※aboveは負荷がかかることが分かったのでv1.4.0以降は新規登録できなくなりました（座標設定ツールを使用してください）
...
|日付|バージョン|説明|備考|
|:---:|:---:|:---|:---|
|2021/05/16|SignalControllerMod-v1.4.0|まれに座標登録時にクラッシュするバグを修正<br>aboveを新規登録できなくなった<br>GUI内で座標が2個以上あれば1個目でも消せるようになった|1.12.2版SignalControllerをリリース|
